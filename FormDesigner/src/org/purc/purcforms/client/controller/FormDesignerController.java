...
public class FormDesignerController implements IFormDesignerListener, OpenFileDialogEventListener {
	...
	if (Context.getCurrentMode() != Context.MODE_DESIGN/*
							    * Context.MODE_QUESTION_PROPERTIES
							    */)
	...
	DeferredCommand.addCommand(new Command() {
		...
		try {
			...
			saveForm(xml, centerPanel.getLayoutXml(), PurcFormBuilder.getCombinedLanguageText(Context
				.getLanguageText().get(formDef.getId())), centerPanel.getJavaScriptSource());
		    ...
		    // saveLanguageText(false); Commented out because we may be
		    // called during change locale where caller needs to have us
		    // complete everything before he can do his stuff, and hence
		    // no more differed or delayed executions.
	...
	/*
	 * if(isOfflineMode()){ final Object obj = leftPanel.getSelectedForm();
	 * if(obj == null){ Window.alert(LocaleText.get("selectSaveItem"));
	 * return; }
	 * 
	 * FormUtil.dlg.setText(LocaleText.get("savingForm"));
	 * FormUtil.dlg.center();
	 ...
	 * try{ String xml = null; xml =
	 * XformBuilder.fromFormDef2Xform((FormDef)obj); xml =
	 * FormDesignerUtil.formatXml(xml);
	 * centerPanel.setXformsSource(xml,formSaveListener == null &&
	 * isOfflineMode()); FormUtil.dlg.hide(); } catch(Exception ex){
	 * FormUtil.displayException(ex); } } }); } else
	 */
	// saveAs();
	...
	DeferredCommand.addCommand(new Command() {
		...
		try {
			...
			public void onResponseReceived(Request request, Response response) {
			    ...
			    /*
			     * int pos =xml.indexOf(PurcConstants.
			     * PURCFORMS_FORMDEF_LAYOUT_XML_SEPARATOR); if(pos >
			     * 0){ xformXml = xml.substring(0,pos); layoutXml =
			     * FormUtil
			     * .formatXml(xml.substring(pos+PurcConstants
			     * .PURCFORMS_FORMDEF_LAYOUT_XML_SEPARATOR.length(),
				...
				layoutXml = FormUtil.formatXml(xml.substring(pos
					+ PurcConstants.PURCFORMS_FORMDEF_LAYOUT_XML_SEPARATOR.length(),
					(pos2 > 0 ? pos2 : (pos3 > 0 ? pos3 : xml.length()))));

				if (pos2 > 0)
				    localeXml = FormUtil.formatXml(xml.substring(pos2
					    + PurcConstants.PURCFORMS_FORMDEF_LOCALE_XML_SEPARATOR.length(),
					    pos3 > 0 ? pos3 : xml.length()));

				if (pos3 > 0)
				    javaScriptSrc = xml.substring(pos3
					    + PurcConstants.PURCFORMS_FORMDEF_JAVASCRIPT_SRC_SEPARATOR.length(), xml
					    .length());
			    } else if (pos2 > 0) {
			    ...
			    // FormUtil.dlg.hide(); //openFormDeffered above
			    // will close it
			}
	...
	DeferredCommand.addCommand(new Command() {
		...
		try {
			...
			// If we are in offline mode, we completely overwrite
			// the form
	...
	DeferredCommand.addCommand(new Command() {
		...
		try {
			...
			for (FormDef form : forms) {
				...
				newFormDef.setLayoutXml(FormUtil.formatXml(LanguageUtil.translate(form.getLayoutXml(),
					xml, false)));
			...
			// refreshObject(); //automatically load widgets on
			// design surface after language switch.
			DeferredCommand.addCommand(new Command() {
			    public void execute() {
				refreshObject();
			    }
	...
	try {
		...
		if (formSaveListener != null) {
			...
			formSaveListener.onSaveLocaleText(formDef.getId(), LanguageUtil.getXformsLocaleText(doc),
	...
	DeferredCommand.addCommand(new Command() {
		...
		DeferredCommand.addCommand(new Command() {
			...
			try {
				...
				SaveFileDialog dlg = new SaveFileDialog(FormUtil.getFileSaveUrl(), xml, formDef
					.getName());
				dlg.center();
			    }
			} catch (Exception ex) {
