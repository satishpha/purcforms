...
public class PropertiesView extends Composite implements IFormSelectionListener, ItemSelectionListener {
	...
	if (propertiesObj != null && Context.allowBindEdit() && !Context.isStructureReadOnly()) {
		...
		txtBinding.setText(name);
		    ...
		    && ((QuestionDef) propertiesObj).getBinding().equals(orgTextDefBinding) /*
											     * startsWith(
											     * "question"
											     * )
											     */) {
		...
		txtBinding.setText(name);
		    ...
		    && ((OptionDef) propertiesObj).getVariableName().equals(orgTextDefBinding) /*
											        * .startsWith
											        * (
											        * "option"
											        * )
	...
	return isAllowedXmlNodeNameStartChar(keyCode) || Character.isDigit(keyCode) || keyCode == '-' || keyCode == '_'
		|| keyCode == '.';
    }
	...
	int code = keyCode;
	return (code == KeyCodes.KEY_ALT || code == KeyCodes.KEY_BACKSPACE || code == KeyCodes.KEY_CTRL
		|| code == KeyCodes.KEY_DELETE || code == KeyCodes.KEY_DOWN || code == KeyCodes.KEY_END
		|| code == KeyCodes.KEY_ENTER || code == KeyCodes.KEY_ESCAPE || code == KeyCodes.KEY_HOME
		|| code == KeyCodes.KEY_LEFT || code == KeyCodes.KEY_PAGEDOWN || code == KeyCodes.KEY_PAGEUP
		|| code == KeyCodes.KEY_RIGHT || code == KeyCodes.KEY_SHIFT || code == KeyCodes.KEY_TAB || code == KeyCodes.KEY_UP);
    }
    ...
    /**
     * Updates the selected object with the new description template as typed by
     * the user.
     */
    ...
    /**
     * Checks if the value entered for Default Value matches the selected
     * Question Data Type.
     * 
     * @return True if and only if the entered value is in sync with the
     *         Question Data Type.
     */
	...
	if (dataType == QuestionDef.QTN_TYPE_REPEAT && questionDef.getDataType() != QuestionDef.QTN_TYPE_REPEAT)
	...
	// txtDescTemplate.setVisible(Context.isStructureReadOnly() ? false :
	// true);
	// btnDescTemplate.setVisible(Context.isStructureReadOnly() ? false :
	// true);
	...
	DeferredCommand.addCommand(new Command() {
		...
		advancedValidationRulesView.setQuestionDef((QuestionDef) propertiesObj);
	    }
	});
    }
	...
	// boolean enable = (enabled && !Context.isStructureReadOnly()) ? true :
	// false;
	...
	if (sender == btnDescTemplate) {
		...
		updateDescTemplate(); // Added for IE which does not properly
				      // throw change events for the desc
				      // template textbox
