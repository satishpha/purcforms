 ...
 *
 */
public class GridDesignGroupWidget extends DesignGroupView implements DragDropListener{
	...
	public void loadWidgets(Element node,FormDef formDef){
		...
		for(int i=0; i<nodes.getLength(); i++){
			...
			if(widget != null && (widget.getWrappedWidget() instanceof GridDesignGroupWidget)){
				((GridDesignGroupWidget)widget.getWrappedWidget()).loadWidgets(element,formDef);
				((GridDesignGroupWidget)widget.getWrappedWidget()).setWidgetSelectionListener(currentWidgetSelectionListener); //TODO CHECK
				if(!widget.isRepeated())
					selectedDragController.makeDraggable(widget, ((GridDesignGroupWidget)widget.getWrappedWidget()).getHeaderLabel());
			}
		}
	}
