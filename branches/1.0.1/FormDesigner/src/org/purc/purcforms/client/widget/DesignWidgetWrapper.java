...
public class DesignWidgetWrapper extends WidgetEx implements QuestionChangeListener, HasAllMouseHandlers, HasAllTouchHandlers {
	...
	public boolean isResizable(){
		if (widget instanceof Label || widget instanceof Hyperlink || widget instanceof CheckBox)
	...
	public Element buildLayoutXml(Element parent, com.google.gwt.xml.client.Document doc){
		...
		value = getCls();
		if(value != null && value.trim().length() > 0)
			node.setAttribute(WidgetEx.WIDGET_PROPERTY_CLASS, value);
		else
			node.removeAttribute(WidgetEx.WIDGET_PROPERTY_CLASS);
