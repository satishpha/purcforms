...
public class TreeItem extends UIObject implements HasHTML {
	...
	public void insertItem(TreeItem item, TreeItem refItem) {
		// Detach item from existing parent.
		if ((item.getParentItem() != null) || (item.getTree() != null)) {
			item.remove();
		}
		...
		// Logical attach.
		item.setParentItem(this);
		int index = children.indexOf(refItem) + 1;
		children.add(index, item); //children.add(item);
		...
		DOM.insertChild(childSpanElem, item.getElement(), index); //DOM.appendChild(childSpanElem, item.getElement());
